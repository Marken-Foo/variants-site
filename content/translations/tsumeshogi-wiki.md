...
- Where gyokukata has multiple moves leading to variations of the same length, the correct one is the one which makes sente use a piece in hand. In addition, gyokukata will prefer to interpose a check with a piece on the board over dropping a piece from the hand (aigoma 合駒), given that both options lead to mate in the same number of moves.
- Gyokukata has in hand all other pieces; in other words, all pieces not on the board or in semekata's hand, excepting the other king.
    ...
    - If all pieces of a type are already accounted for between the board and semekata's hand, gyokukata cannot drop such a piece. For instance, if there are two dragons on the board, gyokukata cannot drop a rook from their hand as aigoma.
    - Gyokukata dropping interposing pieces which are immediately simply captured without fundamentally changing the mating variations are termed "futile interpositions" and are not considered in the move count. However, if the aigoma allows gyokukata to escape mate, or if the mating variations differ with and without the drop, then it is not considered futile. Certain problems may end with the final move being a check that can only be met by interpositions which can be captured for immediate mate, even if not delivered by the currently checking piece; these interpositions are also considered futile.
- Aside from the above, all rules of normal shogi apply (notably, pieces cannot be dropped in places where they have no future legal moves, nifu is not allowed, and semekata cannot mate with a pawn drop (uchifu-dzume) or do perpetual check sennichite.)
